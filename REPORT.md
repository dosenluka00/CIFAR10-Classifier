...
Ovaj projekt uključuje implementaciju i evaluaciju nekoliko pristupa klasifikacije slika na CIFAR-10 skupu podataka koristeći PyTorch. Cilj nije bio samo izmodelirati klasifikator, već also to usporediti više mogućih infrastrukturu i na temelju usporedbe odabrati najprikladniju infrastrukturu.
...
Konačno rješenje sadrži CNN treniran od nule i postiže **78.46% preciznost** na testnom skupu podataka, bez oslanjanja na transfer learning ili predtreniran backbone.
...
Najprije, CIFAR-10 skup podataka sadrži slike veličine 32×32 pixela, dok većina predtreniranigh ImageNet modela su namijenjeni za značajno veće input vrijednosti, stoga za korištenje transfer learninga morali dodatno prilagoditi podatke i veličinu slika.
...
Naposljetku, transfer learning bi donio heuristiku naučenu od ImageNeta, što bi onemogućilo direktnu usporedbu sa baseline modelom, odnosno MLP-om. Treniranjem CNN-a od nule, utjecat konvolucijske arhitekture može biti evaluiran zasebno.

Unatoč svojoj jednostavnosti (malom broju parametara), konačni CNN uči značajne vizualne reprezentacije i postiže snažne performanse bez korištenja vanjski skupova podataka ili predtreniranih težina.
...
Proširenje podataka (eng. data augmentation) primjenjuje se tijekom treninga putem nasumičnih horizontalnih preokreta i nasumičnih izrezivanja s obrubom (eng. padding). Ovo omugućava modelu iskustvo sa više inačica jedne slike i povećava robusnost.

Ključan implementacijski detalj je da su proširenja (eng. augmentation) primjenjene isključivo na skupu podataka za trening. Testni i validacijski skupovi podataka ostaju nepromijenjeni, kako bi osigurali da evalucijske metrike daju uvid u stvarne performanse modela. Deterministička train-validation podjela (eng. split) je korištena kako bi osigurala ponavljanje eksperimenta.

## Faza treninga

Cjelokupan pipeline napredovao je za vrijeme razvoja.

Početni CNN postizao je otprilike 69.7% točnosti na testnom skupu podataka.
...
These improvements increased test accuracy to 78.46% without modifying the underlying CNN architecture.
...
Iz rezultata vidim da CNN postiže značajno bolju točnost, pritom koristeći značajno manji broj parametara.

Unatoč tomu što MLP ima veći broj parametara, on tretira svaki piksel zasebno, nakon procesa image-flatteninga.

S druge strane, CNN koristi prostorni lokalitet (eng. spatial locality) i dijeljenje težina (eng. weights sharing), što ga čini značajno efikasniji obzirom na broj parametara.
